  ...
  const sendMessage = () => {
    if (newMessage.trim() && selectedConversation) {
      ...
      setConversationsList((prev) =>
        prev.map((conv) => (conv.id === selectedConversation.id ? { ...conv, lastMessage: newMessage, time: "now" } : conv))
      );
      setNewMessage("");
              ...
              <View className="mb-4">
                ...
                {selectedConversation.messages.map((message) => (
                  <View key={message.id} className={`flex-row mb-3 ${message.fromUser ? "justify-end" : ""}`}>
                    {!message.fromUser && <Image source={{ uri: selectedConversation.user.avatar }} className="size-8 rounded-full mr-2" />}
