    ...
    <View className="border-b border-gray-100 p-4 bg-white">
      ...
      <View className="flex-row justify-between items-center mt-3">
        ...
        <View className="flex-row items-center">
          {content.length > 0 && (
            <Text className={`text-sm mr-3 ${content.length > 260 ? "text-red-500" : "text-gray-500"}`}>{280 - content.length}</Text>
          )}
          ...
          >
            {isCreating ? (
              ...
              <Text className={`font-semibold ${content.trim() || selectedImage ? "text-white" : "text-gray-500"}`}>Post</Text>
            )}
          </TouchableOpacity>
        </View>
      </View>
