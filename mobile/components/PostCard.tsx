    ...
    <View className="border-b border-gray-100 bg-white">
      <View className="flex-row p-4">
        ...
        <View className="flex-1">
          ...
          {post.content && <Text className="text-gray-900 text-base leading-5 mb-3">{post.content}</Text>}

          {post.image && <Image source={{ uri: post.image }} className="w-full h-48 rounded-2xl mb-3" resizeMode="cover" />}
          ...
          <View className="flex-row justify-between max-w-xs">
            ...
            <TouchableOpacity className="flex-row items-center" onPress={() => onLike(post._id)}>
              {isLiked ? <AntDesign name="heart" size={18} color="#E0245E" /> : <Feather name="heart" size={18} color="#657786" />}

              <Text className={`text-sm ml-2 ${isLiked ? "text-red-500" : "text-gray-500"}`}>{formatNumber(post.likes?.length || 0)}</Text>
            </TouchableOpacity>
